...
class MavenS(MCTS):
    ...
    def evaluate(self, sim_env) -> float:
        ...
        entroy_before_compute=sim_env.occupied['compute_entroy']
        entroy_after_compute = sim_env.compute_compute_entroy()
        entroy_change_compute=entroy_before_compute-entroy_after_compute
        entroy_before_memory=sim_env.occupied['memory_entroy']
        entroy_after_memory = sim_env.compute_memory_entroy()
        entroy_change_memory=entroy_before_memory-entroy_after_memory
        entroy_before_network=sim_env.occupied['network_entroy']
        entroy_after_network = sim_env.compute_network_entroy()
        entroy_change_network=entroy_before_network-entroy_after_network
        # compute reward as resource cost given by weighted (normalized) increase of resources
...
class FutureCoord(MCTS):
    ...
    def evaluate(self, sim_env) -> float:
        ...
        #     return (-1) * (self.alpha * compute/((entroy_change_compute)) + self.beta * memory  + self.gamma * datarate)
        # return (-1/2000) * (self.alpha * compute + self.gamma * datarate)/(abs(entroy_change_compute)+abs(entroy_change_network))
        return (-1) * (1 * compute +1*memory+ 1 * datarate)
