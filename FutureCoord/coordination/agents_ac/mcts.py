...
class MCTS:
    ...
    def select_and_expand(self, sim_env: ServiceCoordination) -> Tuple:
        ...
        while not sim_env.done:
            valid = [action for action in sim_env.valid_actions]
    ...
    def rollout(self, sim_env: ServiceCoordination) -> float:
        ...
        mc_return = 0.0
        obs = sim_env.compute_state_admission()
        ...
        while not sim_env.done1:
            ...
            obs, reward, _, _ = sim_env.place_single_vnf(action)
...
class FutureMavenS(FutureCoord):
    def __init__(self, C: float, max_searches: int, greediness: float, coefficients: Tuple, max_requests: int, rpolicy,
