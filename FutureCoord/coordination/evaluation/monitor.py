...
class CoordMonitor(Monitor):
    ...
    def get_episode_results(self,agent,arrival_rate):
        ep = {}
        info = self.env.info
        ...
        ep['accept_rate'] = total_accepts / self.env.all_requests
        # ep['balanced_accept_rate'] = np.prod([for logs['accepts'] / logs['requests'] for logs in info])
        ep['jain_index']=self.env.compute_jain()
        ep['reward_no_cost']=self.env.compute_reward_no_cost()
        # ep['jain_accept_index']=self.env.compute_jain_accepts()
        # ep['reward_cost']=sum(self.reward_list)
        ep['agent_name']=agent.name
        ep['arrival_rate']=arrival_rate
        ...
        ep['mean_cutil'] =self.c_util[-1]
        ep['mean_mutil'] = self.m_util[-1]
        ep['mean_dutil'] = self.d_util[-1]
