...
class ServiceCoordination(gym.Env):
    ...
    def update_actions(self) -> None:
        ...
        compute = np.asarray(list(self.computing.values()))
        max_compute = np.asarray(list(c for _, c in self.net.nodes('compute')))
        compute = compute / np.sum(max_compute)

        memory = np.asarray(list(self.memory.values()))
        max_memory = np.asarray(list(m for _, m in self.net.nodes('memory')))
        memory = memory / np.sum(max_memory)
        # compute aggregated resource vector (accounts for multiple resources)
        cdemand=[0 for node in self.net.nodes()]
        mdemand=[0 for node in self.net.nodes()]
        resources = 0.5 * compute + 0.5 * memory
        for i in self.net.nodes():
            if i in mdemands.keys():
                mdemand[i]=mdemands[i]/self.MAX_MEMORY
            else:
                mdemand[i]=10
        for i in self.net.nodes():
            if i in cdemands.keys():
                ...
                cdemand[i]=10
        resource_cost=[0 for i in self.net.nodes()]
        for i in self.net.nodes():
            resource_cost[i]=2*cdemand[i]+mdemand[i]
        resource_final=resources/resource_cost

        # print(resources)
        # cdemand = np.asarray(list(cdemands.values()))
        # max_compute = np.asarray(list(m for _, m in self.net.nodes('compute')))
        # cdemand = cdemand / np.sum(max_compute)
        # mdemand = np.asarray(list(mdemands.values()))
        # max_memory = np.asarray(list(m for _, m in self.net.nodes('memory')))
        # mdemand = mdemand / np.sum(max_memory)
        # resources_cost=[0 for node in self.net.nodes]
        # for node in self.net.nodes:
        #     if node in valid_nodes:
        #         resources_cost[node]=0.5 * cdemand[node] + 0.5 * mdemand[node]
        #     else:
        #         resources_cost[node]=1
        # print(resources_cost)
    ...
    def render(self, mode: str = 'textual', close: bool = False) -> str:
        ...
        if self.pos is None:
            self.pos = nx.spring_layout(self.net, iterations=10)
