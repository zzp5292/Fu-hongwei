...
parser = argparse.ArgumentParser()
# arguments to specify the experiment, agent & evaluation
parser.add_argument('--experiment', type=str, default='./data/experiments/abilene_2/trace.yml')
parser.add_argument('--agent', type=str, default='./data/configurations_ac/MaskablePPO.yml')
parser.add_argument('--episodes', type=int, default=10)
parser.add_argument('--logdir', type=str, default='./results_test_new_environmet/results_dalunwen/max_reward_2/0.35')
...
if __name__ == '__main__':
    ...
    with open(Path(args.experiment)) as file:
        ...
        exp = munchify(exp)
    logdir = Path(args.logdir+"/"+config.name+"/"+str(exp.arrival_rate))
    if logdir.exists():
    ...
    for ep in range(1):
        train_rng = np.random.default_rng(seed= ep)
        sim_rng = np.random.default_rng(seed=ep)
        ...
        train_process = utils.setup_process(train_rng, exp, services, eday, sdays, exp.load, exp.datarate, exp.latency,exp.arrival_rate)
        eval_process = utils.setup_process(eval_rng, exp, services, eday, sdays, exp.load, exp.datarate, exp.latency,exp.arrival_rate)
        ...
        sim_process = utils.setup_sim_process(rng, sim_rng, exp, args, eval_process, services, eday, sdays,
                                              exp.sim_load, exp.sim_datarate, exp.sim_latency,exp.arrival_rate)
        ...
        env = utils.setup_simulation_ac_queue_reject(config, exp.overlay, train_process, vnfs, chains)
        ...
        agent = utils.setup_agent_ac(config, monitor, seed=ep)
        ...
        if "MaskablePPO" in args.agent:
        # evaluate agent on evaluation environment
            ep_results = utils.evaluate_episode_MaskablePPO(agent, monitor, sim_process,exp.arrival_rate)
        else:
            ep_results=utils.evaluate_episode(agent, monitor, sim_process,exp.arrival_rate)
